...
SlamGMapping::SlamGMapping():
  ...
  scan_nonStruct_filter_sub_(NULL), scan_nonStruct_filter_(NULL), transform_thread_(NULL)
{
  seed_ = time(NULL);
  init();
}
...
void SlamGMapping::init()
{
  ...
  if(!private_nh_.getParam("occ_thresh", occ_thresh_))
    occ_thresh_ = 0.25;
  // Set this param to "true" to read the nonstruct_scan topic
  if(!private_nh_.getParam("listen_nonstruct_scan", listen_NonStruct_scan_))
    listen_NonStruct_scan_ = false;
  // Mimimum value for non struct values to be taken into consideration
  if(!private_nh_.getParam("occ_nonstruct_thresh", occ_nonStruct_thresh_))
    occ_nonStruct_thresh_ = 0.01;
  // Number of neighbour cells to search for non struct values
  if(!private_nh_.getParam("nonstruct_neighbours", occ_nonStruct_neighbours_))
    occ_nonStruct_neighbours_ = 2;
...
void SlamGMapping::startLiveSlam()
{
  ...
  if (listen_NonStruct_scan_) {
    // Listen to the nonstruct_scan topic
    scan_nonStruct_filter_sub_ = new message_filters::Subscriber<sensor_msgs::LaserScan>(node_, "nonstruct_scan", 5);
    scan_nonStruct_filter_ = new tf::MessageFilter<sensor_msgs::LaserScan>(*scan_nonStruct_filter_sub_, tf_, odom_frame_, 5);
    scan_nonStruct_filter_->registerCallback(boost::bind(&SlamGMapping::laserCallbackNonStruct, this, _1));
  }
...
bool
SlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose, bool nonStruct)
{
...
{
    laserCallbackImpl(scan, false);
}

void
SlamGMapping::laserCallbackNonStruct(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    laserCallbackImpl(scan, true);
}

void
SlamGMapping::laserCallbackImpl(const sensor_msgs::LaserScan::ConstPtr& scan, bool nonStruct)
{
  laser_count_++;
  ...
  if(addScan(*scan, odom_pose, nonStruct))
  {
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
      ...
      n = n->parent)
  {
    ...
    matcher.registerScan(smap, n->pose, &((*n->reading)[0]), n->reading->isNonStruct());
  }
...
//  if (nonStruct)
//      return;
  int maxX = smap.getMapSizeX();
  int maxY = smap.getMapSizeY();
  for(int x=0; x < maxX; x++)
  {
    for(int y=0; y < maxY; y++)
    {
      ...
      else if(occ > occ_thresh_)
      {
        ...
        if (cell.nonStructOccupancy() > occ_nonStruct_thresh_)
            // If cell is non structural then put value 55
            map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = 55;
        else if (nearNonStructCell(smap, maxX, maxY, p))
            // If some neighbour cell is non structural then put value 55
            map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = 55;
        else
...
bool SlamGMapping::nearNonStructCell(GMapping::ScanMatcherMap& map, int maxX, int maxY, GMapping::IntPoint &p) const
{
    for (int x = p.x - occ_nonStruct_neighbours_; x <= p.x + occ_nonStruct_neighbours_; x++)
        if (x >= 0 && x < maxX)
            for (int y = p.y - occ_nonStruct_neighbours_; y <= p.y + occ_nonStruct_neighbours_; y++)
                if (y >= 0 && y < maxY && x != p.x && y != p.y)
                {
                    GMapping::IntPoint p2(x, y);
                    if (map.cell(p2).nonStructOccupancy() > occ_nonStruct_thresh_)
                        return true;
                }
    return false;
}
