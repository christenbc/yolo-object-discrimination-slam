...
double ScanMatcher::registerScan(ScanMatcherMap& map, const OrientedPoint& p, const double* readings, bool nonStruct){
	if (!m_activeAreaComputed)
		...
		if (m_generateMap){
			...
			for (int i=0; i<line.num_points-1; i++){
				...
				double e=-cell.entropy();
                                cell.update(false, nonStruct, Point(0,0));
				e+=cell.entropy();
				esum+=e;
			}
			if (d<m_usableRange){
				double e=-map.cell(p1).entropy();
                                map.cell(p1).update(true, nonStruct, phit);
